...
export const AnimatedImageView = styled(Animated.View).attrs(({ scrollY }) =>({
    pointerEvents:"none" ,
}))`
    position:absolute;
    top: 0;
...
export const AnimatedImage = styled(Animated.Image).attrs(({ photo }) =>({
    source: {uri: photo },
}))`
    position: absolute;
    top: 0;
...
export const MarginTop = styled.View`
    margin-top:${-HEADER_MIN_HEIGHT/2}px;
`;
